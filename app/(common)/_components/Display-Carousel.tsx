...
export default function DisplayCarousel() {
  ...
  return (
    <div className="px-6 md:px-10 lg:px-16 py-6 lg:py-16 w-full flex-1 flex justify-center items-center">
  ...
  function getCircularButtons(noElements: number) {
    ...
    for (let i = 0; i < noElements; i++) {
      buttons.push(
        <button
          key={i}
          className="h-3 w-3 bg-gray-600 opacity-60 rounded-full"
          onClick={() => setActiveIndex(i)}
        >
          {activeIndex === i && <div className="w-full h-full bg-orange-800 rounded-full" />}
        </button>
      );
    }
